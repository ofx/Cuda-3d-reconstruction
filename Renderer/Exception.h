...
public:
	ConstructorException(const std::string &arg, const char *file, int line, bool graceful = false) : std::runtime_error(arg), m_Graceful(graceful)
	{
		std::ostringstream o;
		o << file << ":" << line << ": " << arg;
...
#define throw_line_graceful() throw ConstructorException("", __FILE__, __LINE__, true);
